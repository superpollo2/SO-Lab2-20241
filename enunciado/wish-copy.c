...
void shellPrompt()
{
    ...
    printf("wish > %s$ ", getcwd(currentDirectory, 1024));
}
...
void pipeHandler(char *args[])
{
    ...
    while (args[j] != NULL && end != 1)
    {
        ...
        if (pid == 0)
        {
            ...
            {
                if (num_cmds % 2 != 0)
                { // para número impar de comandos
                    dup2(filedes[0], STDIN_FILENO);
                }
                else
                { // para número par de comandos
